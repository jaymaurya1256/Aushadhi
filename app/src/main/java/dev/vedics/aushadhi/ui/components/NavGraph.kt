...
fun AppNavigation() {
    ...
    NavHost(
        ...
        composable(route = AUSHADHI_SCREEN) {
            AushadhiScreen(items = List(20) { "Item #$it aushadhi" }, 100.dp, navController)
        }
        composable(route = PATIENT_SCREEN) {
            PatientScreen(items = List(20) { "Item #$it disease" }, 100.dp, navController)
        }
        composable(route = DISEASE_SCREEN) {
            DiseaseScreen(items = List(20) { "Item #$it patient" }, 100.dp, navController)
        }
    }
}
